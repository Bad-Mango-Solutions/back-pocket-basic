...
- **FR-DC0 (cross-cutting path resolution)** Every `<path>` / `<bootimage>` argument accepted by any `disk` subcommand — current (`disk create`, `disk info`) and future (`disk insert`, `disk eject`, `disk flush`, `disk list` when echoing image paths) — **must** be resolved through `IDebugPathResolver.TryResolve` before being passed to `DiskImageFactory`, `File.Exists`, `File.Move`, or any other filesystem API. The resolver supports the `library://`, `app://`, absolute, and relative path forms; the `embedded://` scheme is intentionally rejected for disk I/O. The canonical slug for disk images under the user library is `library://disks/<name>` (matches `LibrarySettingsViewModel.DiskImagesPath = "{Library}/disks"` and the profile examples in §6.4). When a subcommand reports an image path back to the user (e.g. `disk info`, `disk list`, error messages from `disk create`) it should display both the original `library://` form (when supplied) and the resolved absolute path so failures are diagnosable.
...
- **FR-DC5** `disk eject <slot>:<drive>` — flushes dirty state, dismounts. Fails clearly for non-removable units. Lands with issue 6.
  - **Disk II behaviour (today):** the physical Disk II had no electrical interlock on the drive door — opening the latch during a read or write was always *physically* permitted (even if it scrambled whatever was being written). The runtime `disk eject` therefore must never refuse on the basis of motor state, and must never crash the emulator when bookkeeping (e.g. the `MountedDiskRegistry`) is in a torn-down or racing state; the eject of the controller's own media reference is what matters.
  - **SmartPort / UniDisk 3.5" behaviour (future, with issue 12):** the UniDisk 3.5" exposes `STATUS` / `CONTROL` SmartPort *eject events* the host can observe and act on. Real hardware refused to eject while the drive was being accessed; software eject (the GS Finder "Put Away" path, or a `CONTROL` `EJECT` SmartPort call) only succeeded once the unit was idle. Because the front-panel eject button on the UniDisk 3.5" was a soft button routed through the same firmware, a paperclip-sized hole in the button face provided a mechanical *force-eject* override that bypassed the firmware interlock entirely. We intend to model this with:
    - **`disk eject <slot>:<drive>`** (current command) — for SmartPort 3.5" units, this is the *soft* eject: it raises the SmartPort eject event, waits for the firmware-side acknowledgement (subject to a short timeout), then dismounts. Refused while a `READ_BLOCK` / `WRITE_BLOCK` is in flight.
    - **`disk force-eject <slot>:<drive>`** (future, lands with issue 12) — paperclip-equivalent mechanical override that bypasses the SmartPort interlock and dismounts immediately, even mid-transfer. Always succeeds against ejectable units; carries a clear "data loss possible" warning in the command output.
    - For Disk II units, `disk force-eject` is an alias of `disk eject` because the hardware never had an interlock to bypass.
...
| 12 | Extend `disk insert` / `disk eject` to SmartPort 3.5" UniDisk volumes (removable units only); add `disk force-eject` for the paperclip-style firmware-interlock override (FR-DC5) | Mount/eject works through the same console commands; non-removable units reject mount/eject with a clear error; `disk force-eject` always succeeds against ejectable units even mid-transfer. |
